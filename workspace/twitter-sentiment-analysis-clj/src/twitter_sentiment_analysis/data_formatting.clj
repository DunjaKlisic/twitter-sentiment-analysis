...
(defn reduce-features
  [path-positive path-negative path-dataset]
  (with-open [out-file (io/writer path-dataset)
              rdrp (reader path-positive)
              rdrn (reader path-negative)]
    (csv/write-csv out-file [["text" "sentiment"]])
    (doseq [line (line-seq rdrp)]
      (csv/write-csv
        out-file 
        [[((comp strip-repeated-letters
                 strip-urls
                 strip-usernames
                 strip-emoticons
                 strip-commas
                 strip-apostrophe
                 strip-double-apostrophe)
            (:text (json/read-str line :key-fn keyword)))
          "positive"]]))
